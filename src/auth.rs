...
impl CasdoorSDK{
	...
	pub fn new(
		...
		let front_endpoint = front_endpoint.unwrap_or(&endpoint.replace(":8000", ":7001")).to_owned();
		CasdoorSDK{
	...
	pub async fn get_auth_link(
		...
		let _res =client.get(&url).form(&param).send().await?.error_for_status()?;
		let mut ret = url + "?";
	...
	pub async fn get_user_by_name(&self, name: &str) -> Result<Option<User>, Box<dyn std::error::Error>> {
	...
	async fn modify_user(&self, action: &str, mut user: User) -> Result<Response, Box<dyn std::error::Error>> {
...
fn test_get_auth_link(){
	...
	assert_eq!(url, "http://127.0.0.1:7001/login/oauth/authorize?client_id=5dfdb2b205c8d7efdf26&redirect_uri=http://localhost:9000/callback&response_type=code&scope=read&state=testsdk_a1")
}
